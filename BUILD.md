...
```
target/notice-plugin-0.0.2.jar
```
...
```
build/libs/notice-plugin-0.0.2.jar
```
...
```
target/
├── notice-plugin-0.0.2.jar          # 最終的なプラグインファイル
...
生成された`notice-plugin-0.0.2.jar`をMinecraftサーバーの`plugins/`フォルダにコピーして使用してください。
