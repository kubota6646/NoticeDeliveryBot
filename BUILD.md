...
```
target/notice-plugin-0.0.3.jar
```
...
```
build/libs/notice-plugin-0.0.3.jar
```
...
```
target/
├── notice-plugin-0.0.3.jar          # 最終的なプラグインファイル
...
生成された`notice-plugin-0.0.3.jar`をMinecraftサーバーの`plugins/`フォルダにコピーして使用してください。
